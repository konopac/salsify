...
public class Codec implements ICodec {
	...
	public Optional<BufferedImage> decode(Optional<BufferedImage> state, byte[] encodedFrame) {
		...
		try {
			decodedFrame = Optional.of(ImageIO.read(new ByteArrayInputStream(encodedFrame)));
	...
	/**
	 * Takes image and converts to a raw byte array containing an image in the jpeg
	 * format.
	 *
	...
	private Optional<byte[]> convertToJpeg(BufferedImage toConvert, float quality) {
		...
		try (ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
			...
			System.out.println("Encode: Error occured while encoding image to jpeg: " + exception.toString());
		}
	...
	private BufferedImage calculateDifference(BufferedImage before, BufferedImage after) {
		...
		// result image
		BufferedImage result = new BufferedImage(before.getWidth(), before.getHeight(), BufferedImage.TYPE_3BYTE_BGR);

		// loop and calculate difference
		for (int y = 0; y < result.getHeight(); y++) {
			for (int x = 0; x < result.getWidth(); x++) {
				
				int pixel = calculatePixelDifference(before.getRGB(x, y), after.getRGB(x, y));
				result.setRGB(x, y, pixel);
				
				if (x == 96 && y == 128) {
					System.out.println("difference before compression = " + new Color(pixel).toString());
				}
			}
		}
	...
	/**
	 * Calculates the difference between two pixel components. The difference is
	 * always expressed as an addition with modulo 256 to do a wrap around.
	 *
	...
	private int calculatePixelDifference(int rgbOne, int rgbTwo) {
		...
		BiFunction<Integer, Integer, Integer> componentDifference = (componentOne, componentTwo) -> {
			int result;
	...
	private BufferedImage addDifference(BufferedImage state, BufferedImage difference) {
		...
		// result image
		BufferedImage result = new BufferedImage(state.getWidth(), state.getHeight(), BufferedImage.TYPE_3BYTE_BGR);

		BufferedImage expected = null;
		try {
			expected = ImageIO.read(new File("/home/philipp/Studium/workspaces/netzwerke2/Salsify/images/image2.jpg"));
		} catch (IOException e) {
		}
		...
		for (int y = 0; y < result.getHeight(); y++) {
			for (int x = 0; x < result.getWidth(); x++) {
				int pixelExpected = expected.getRGB(x, y);
				int pixel = addPixelDifference(state.getRGB(x, y), difference.getRGB(x, y));
				
				if ((0xFFFFFF & pixel) != (0xFFFFFF & pixelExpected)) {
					System.out.println("Wrong: x = " + x + "; y = " + y + "; pixel = " + Integer.toBinaryString(0xFFFFFF & pixel) + "; expected = " + Integer.toBinaryString(0xFFFFFF & pixelExpected) + ";");
				}
				
				if (x == 96 && y == 128) {
					System.out.println("expected = " + new Color(pixelExpected).toString());
					System.out.println("state = " + new Color(state.getRGB(x, y)).toString());
					System.out.println("difference = " + new Color(difference.getRGB(x, y)).toString());
					System.out.println("result = " + new Color(pixel).toString());
				}
				result.setRGB(x, y, pixel);
			}
		}
	...
	/**
	 * This performs the reverse operation of the operation performed in calculated
	 * pixel difference.
	 * 
	...
	private int addPixelDifference(int rgbState, int rgbDifference) {
		...
		int red = (getRed(rgbState) + getRed(rgbDifference) - 128) % 256;
		int green = (getGreen(rgbState) + getGreen(rgbDifference) - 128) % 256;
		int blue = (getBlue(rgbState) + getBlue(rgbDifference) - 128) % 256;
